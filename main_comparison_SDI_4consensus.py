...
def run_analysis(config, MatMat, EucMat, dict_df, comp_parc, nbProcs):
    ...
    for g,group in enumerate(gp):
        for d,dw in enumerate(dwi):
            ...
            hemii = np.ones(np.shape(EucMat[:,:,idxs])[0])
            hemii[int(len(hemii)/2):] = 2
            nbins = 42
            tmp, G_unif = fcn_groups_bin.fcn_groups_bin(MatMat[:,:,idxs], np.mean(EucMat[:,:,idxs],axis=2), hemii, nbins) 
            G_dist[:,:,k] = tmp*np.average(MatMat[:,:,idxs], axis=2)
            ...
            P_ind[:,k], Q_ind[:,:,k], Ln_ind, An_ind = ML.cons_normalized_lap(G_dist[:,:,k], np.mean(EucMat[:,:,idxs],axis=2),  plot=False)
    ...
    np.save(os.path.join(config["Parameters"]["output_dir"], 'epilepsy','Qind_%d_beforeRot_newProcu'%(k)), Q_ind)
    np.save(os.path.join(config["Parameters"]["output_dir"], 'epilepsy','Qind_rot_%d_afterRot_wei_newProcu'%(k)), Q_ind_rot)
    np.save(os.path.join(config["Parameters"]["output_dir"], 'epilepsy','Pind_%d_beforeRot_wei_newProcu'%(k)), P_ind)

   
    ls_lat = []; SDIr={}; SDI_surr_r={}  
    SDI={}; SDI_surr={} 
    ...
    for g,group in enumerate(gp):
        for d,dw in enumerate(dwi):  
            ls_lat = []; 
            SDIr = np.zeros((114,17)); SDI = np.zeros((114,17)) 
            SDI_tmpr = np.zeros((114, len(X_RS_allPat))); SDI_tmp = np.zeros((114, len(X_RS_allPat)))
            for p in np.arange(len(X_RS_allPat)):
                ...
                #PSD,NN, Vlow, Vhigh = sdi.get_cutoff_freq(Q_ind[:,:,k], X_RS[idx_ctx,:,:])
                #SDI_tmp[:,p], X_c_norm, X_d_norm, SD_hat = sdi.compute_SDI(X_RS[idx_ctx,:,:], Q_ind[:,:,k])
                PSDr,NNr, Vlowr, Vhighr = sdi.get_cutoff_freq(Q_ind_rot[:,:,k], X_RS[idx_ctx,:,:])
                SDI_tmpr[:,p], X_c_norm, X_d_norm, SD_hat = sdi.compute_SDI(X_RS[idx_ctx,:,:], Q_ind_rot[:,:,k])
                PSD,NN, Vlow, Vhigh = sdi.get_cutoff_freq(Q_ind[:,:,k], X_RS[idx_ctx,:,:])
                SDI_tmp[:,p], X_c_norm, X_d_norm, SD_hat = sdi.compute_SDI(X_RS[idx_ctx,:,:], Q_ind[:,:,k])
            ls_lat = np.array(ls_lat)
            np.save(os.path.join(config["Parameters"]["output_dir"], 'epilepsy','ls_lat.npy'), ls_lat)

            SDI = SDI_tmp; SDIr = SDI_tmpr 
            np.save(os.path.join(config["Parameters"]["output_dir"], 'epilepsy','SDI_%d_afterRot'%(k)), SDIr)
            np.save(os.path.join(config["Parameters"]["output_dir"], 'epilepsy','SDI_%d_beforeRot'%(k)), SDI)

            fig,ax = plt.subplots(1,1)
            ax.scatter(np.mean(SDI,axis=1), np.mean(SDIr,axis=1))

            ### Surrogate part
            SDI_surr = sdi.surrogate_sdi(Q_ind[:,:,k], Vlow, Vhigh, config,  nbSurr=10, example=False) # Generate the surrogate 
            SDI_surr_r = sdi.surrogate_sdi(Q_ind_rot[:,:,k], Vlow, Vhigh, config,  nbSurr=10, example=False) # Generate the surrogate 
            np.save(os.path.join(config["Parameters"]["output_dir"], 'epilepsy','SDIsurr_%d_afterRot'%(k)), SDI_surr_r)
            np.save(os.path.join(config["Parameters"]["output_dir"], 'epilepsy','SDIsurr_%d_beforeRot'%(k)), SDI_surr)
            ...
            for l, lat in enumerate(np.unique(ls_lat)):
                ...
                #t=0
                    ...
                    plot_rois_pyvista(tmp, config["Parameters"]["scale"], config, vmin=-1, vmax=1, label='SDI_th%d_%s_%d_beforeRotation'%(surr_thresh[t]['threshold'], lat, k))
     
     
                    SDI_tmp = SDIr; SDI_surr_tmp = SDI_surr_r
                    surr_thresh = sdi.select_significant_sdi(SDI_tmp[:,np.where(ls_lat==lat)[0]], SDI_surr_tmp[:,:,np.where(ls_lat==lat)[0]])
                    ...
                    #t=0
                        ...
                        #tmp = surr_thresh[t]['mean_SDI']
                        nbROIsig.append(len(np.where(np.abs(surr_thresh[t]['SDI_sig']))[0]))
                        plot_rois_pyvista(tmp, config["Parameters"]["scale"], config, vmin=-1, vmax=1, label='SDI_th%d_%s_%d_afterRotation'%(surr_thresh[t]['threshold'], lat, k))
